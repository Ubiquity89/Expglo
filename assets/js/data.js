...
const EXPGLO_STORE = {
    ...
    return [
      {
        id: "fund",
        name: "EXPGLO FUND",
        category: "Founder × Investor Ecosystem",
  ...
  getEvents: function () {
    ...
    const saved = localStorage.getItem("expglo_events");
    if (!saved) {
      localStorage.setItem("expglo_events", JSON.stringify(defaultEvents));
  ...
  getNews: function () {
    const defaultNews = [
      ...
      },
      {
        ...
        excerpt:
          "EXPGLO School unveils a structured 10-week curriculum focused squarely on product traction, customer discovery, and live pitch readiness.",
        content:
      ...
      {
        id: "news-3",
        title: "EXPGLO School partners with premier technical universities",
        category: "Education",
        date: "April 14, 2025",
    ...
    const saved = localStorage.getItem("expglo_news");
    if (!saved) {
      localStorage.setItem("expglo_news", JSON.stringify(defaultNews));
      return defaultNews;
    }
    return JSON.parse(saved);
  },
  ...
  saveEnquiry: function (enquiryData) {
    const enquiries = JSON.parse(
      localStorage.getItem("expglo_enquiries") || "[]",
    );
    const newEntry = {
    ...
    localStorage.setItem("expglo_enquiries", JSON.stringify(enquiries));
    return newEntry;
  },

  getEnquiries: function () {
    return JSON.parse(localStorage.getItem("expglo_enquiries") || "[]");
  },
  ...
  updateEnquiryStatus: function (id, newStatus) {
    ...
    localStorage.setItem("expglo_enquiries", JSON.stringify(updated));
    return updated;
  },
  ...
  saveApplication: function (applicationData) {
    const applications = JSON.parse(
      localStorage.getItem("expglo_applications") || "[]",
    );
    const newApp = {
    ...
    localStorage.setItem("expglo_applications", JSON.stringify(applications));
    return newApp;
  },

  getApplications: function () {
    return JSON.parse(localStorage.getItem("expglo_applications") || "[]");
  },

  // School Registrations
  saveEnrollment: function (enrollmentData) {
    const enrollments = JSON.parse(
      localStorage.getItem("expglo_enrollments") || "[]",
    );
    const newEnroll = {
    ...
    localStorage.setItem("expglo_enrollments", JSON.stringify(enrollments));
    return newEnroll;
  },
};

// Seed sample enquiries if empty
if (!localStorage.getItem("expglo_enquiries")) {
  localStorage.setItem(
    ...
    JSON.stringify([
      ...
      },
      {
        ...
        message:
          "Interested in setting up an EXPGLO School chapter on our college campus for over 3,000 engineering students.",
        status: "Contacted",
